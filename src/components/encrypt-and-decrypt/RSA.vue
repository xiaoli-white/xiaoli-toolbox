...
export default {
  ...
  mounted() {
    ...
    this.subTabActiveKey = localStorage.getItem('rsa_sub_tab_key') || "1";
  },
  watch: {
    subTabActiveKey(val) {
      localStorage.setItem('rsa_sub_tab_key', val);
    }
  },
  methods: {
    generateKeys() {
...
<template>
  ...
  <a-tabs v-model:activeKey="subTabActiveKey" class="sub-tabs" centered>
